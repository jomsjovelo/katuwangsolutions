      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-5 py-12 md:py-20 space-y-10">
        ...
        <div className="prose prose-slate max-w-none space-y-8">
          ...
          <section className="space-y-4">
            ...
            <p className="text-slate-600 leading-relaxed">
              Meta Pixel is enabled on the website. It sends <code>PageView</code> for page visits; <code>ViewContent</code> for public module views; <code>InitiateCheckout</code> when module onboarding opens; and <code>CompleteRegistration</code> after account registration succeeds. It may also send limited custom events for module discovery, registration-button use, role or module selection, onboarding stage views, payment-instruction views, Messenger payment-button use, and a user&apos;s acknowledgement that a payment screenshot was sent. Event parameters are limited to non-personal context such as module ID, module category, CTA source, discovery type, role type, and stage name. We do not send registration-field values, names, email addresses, phone numbers, addresses, birth dates, gender, business records, payment screenshots, tenant IDs, user IDs, referral codes, raw URLs, or raw query strings to Meta through these application events. Meta may process other browser, device, network, cookie, or similar information under Meta&apos;s own terms, settings, and privacy practices.
            </p>
            <p className="text-slate-600 leading-relaxed">
              When a new business account is created, Katuwang Solutions may store a limited first-party acquisition record with the tenant account to understand registration and activation performance. This record is limited to the landing-page path, sanitized UTM source, medium, campaign, and content values when present, and the registration CTA source. It does not store fbclid, utm_term, a full referrer URL, a raw query string, or registration-form values for this purpose.
            </p>
          </section>
