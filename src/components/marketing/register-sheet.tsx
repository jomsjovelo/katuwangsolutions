...
export function RegisterSheet({ open, onClose, initialAppId = '' }: RegisterSheetProps) {
  ...
  useEffect(() => {
    if (open) setSelectedId(initialAppId);
  }, [open, initialAppId]);
    ...
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative bg-white rounded-t-[28px] shadow-2xl animate-in slide-in-from-bottom-full duration-300 ease-out max-h-[90dvh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 flex-shrink-0">
          <div>
            <h3 className="text-lg font-black text-slate-900 tracking-tight">Anong uri ng negosyo mo?</h3>
            <p className="text-xs text-slate-500 font-medium">Pumili sa 16 apps — ₱99/buwan.</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-5 py-4 border-t border-slate-100 bg-white flex-shrink-0">
          ...
          >
            {selectedId ? 'Ituloy ang Pagpaparehistro' : 'Pumili ng App'}
