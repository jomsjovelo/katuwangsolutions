...
import { enqueueOfflineTransaction, getQueuedTransactions, removeQueuedTransaction } from '@/lib/offline-budget-queue';
...
export async function syncOfflineBudgetQueue(tenantId: string): Promise<number> {
  const items = await getQueuedTransactions(tenantId);
      ...
      await runTransactionResilient(db, async (transaction) => {
        ...
        if (!masterAccountSnap.exists()) {
          transaction.set(masterAccountRef, {
            ...
            name: 'Main Cash Register',
            type: 'asset',
            balance: item.type === 'income' ? item.amountCentavos : -item.amountCentavos,
            isActive: true,
            createdAt: serverTimestamp(),
          });
        } else {
          transaction.update(masterAccountRef, {
            balance: increment(item.type === 'income' ? item.amountCentavos : -item.amountCentavos),
            updatedAt: serverTimestamp(),
          });
        }
      });

      await removeQueuedTransaction(item.queueId);
      syncedCount++;
    } catch (err) {
      console.error(`[Budget Mo] Failed to sync queued item ${item.queueId}:`, err);
    }
  }
